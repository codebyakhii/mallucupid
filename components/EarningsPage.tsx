...
const EarningsPage: React.FC<EarningsPageProps> = ({ onBack, earnings, onRequestWithdrawal, pendingWithdrawals, userProfile }) => {
  const [withdrawAmount, setWithdrawAmount] = useState('');
  const [showSuccessModal, setShowSuccessModal] = useState(false);
  const [error, setError] = useState('');

  // Use the verified balance from the user profile state which is updated by Admin
  const availableBalance = userProfile.balance || 0;
  const totalEarned = earnings.reduce((sum, e) => sum + e.amount, 0);
  const pendingAmount = pendingWithdrawals.reduce((sum, w) => w.status === 'pending' ? sum + w.amount : sum, 0);
  ...
  const handleWithdraw = () => {
    ...
    if (amount < 500) {
      setError('Minimum withdrawal ₹500');
      return;
    }
      ...
      <div className="p-6 space-y-8">
        ...
        <div className="bg-black rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-5xl font-black tracking-tighter mb-8">₹{availableBalance.toLocaleString()}</h2>
            ...
            <div className="space-y-4">
              ...
              <button 
                ...
                className="w-full py-4 bg-white text-black rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-xl active:scale-95 transition-all"
              >
                Request Withdrawal
              </button>
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {earnings.length === 0 ? (
              ...
              [...earnings].sort((a,b) => b.timestamp - a.timestamp).map(e => (
                <div key={e.id} className="bg-white p-5 rounded-[2rem] border border-orange-50 flex items-center justify-between shadow-sm">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${e.type === 'purchase' ? 'bg-blue-50 text-blue-500' : 'bg-orange-50 text-orange-500'}`}>
                      {e.type === 'purchase' ? '🛒' : '👑'}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                         <p className="text-[10px] font-black uppercase text-gray-800">@{e.username}</p>
                         <span className="text-[8px] font-bold text-gray-400 uppercase px-1.5 py-0.5 bg-gray-50 rounded border border-gray-100">{e.type}</span>
                      </div>
                      <p className="text-[8px] font-bold text-gray-400 uppercase mt-0.5">ID: {e.targetId} • {new Date(e.timestamp).toLocaleDateString()}</p>
                    </div>
                  </div>
                  <p className="text-sm font-black text-[#006400]">₹{e.amount}</p>
                </div>
              ))
            )}
          </div>
            ...
            <div className="space-y-3">
              {pendingWithdrawals.map(w => (
                <div key={w.id} className="bg-white/60 p-5 rounded-[2rem] border border-orange-50/50 flex items-center justify-between">
                  <div>
                    <p className="text-[9px] font-black uppercase text-gray-500">{new Date(w.timestamp).toLocaleString()}</p>
                  ...
                  <p className="text-sm font-black text-gray-800">-₹{w.amount}</p>
                </div>
              ))}
            </div>
          </div>
